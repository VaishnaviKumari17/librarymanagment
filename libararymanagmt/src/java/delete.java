...
public class delete extends HttpServlet {
    ...
    protected void doDelete(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            PreparedStatement ps = con.prepareStatement("DELETE FROM stock WHERE bookid = ?");
            ps.setString(1, bookid);

            int count = ps.executeUpdate();

            if (count > 0) {
                out.print("<h3>Book Informanation deleted successfully. Please check your database to confirm.</h3>");
